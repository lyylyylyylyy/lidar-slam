            ...
            int& beam_number)
    {
       ...
       int index = startIndex;
        ...
        for (int i = 0; i < beam_number; ++i) {
            ...
            tf::Quaternion rot = start_rot.slerp(end_rot, (float)i / beam_number);
            tf::Pose current_pos = tf::Pose(rot, pos);

            // 扫描点坐标转换xyz，2D雷达，z=0
            float x = ranges[index] * cos(angles[index]);
            float y = ranges[index] * sin(angles[index]);
            tf::Vector3 scan_points(x, y, 0.0);

            // 矫正扫描点
            tf::Vector3 corrected_scan_point = current_pos * scan_points;
